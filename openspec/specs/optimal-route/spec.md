...
## Purpose
The Optimal Route service provides automated, high-performance shortest path calculation for vehicle tow truck dispatch across district road networks. Using a multi-source Dijkstra algorithm implementation with a priority queue, it determines the nearest tow truck depot to an accident location and reconstructs the optimal traversal path and cumulative distance.
